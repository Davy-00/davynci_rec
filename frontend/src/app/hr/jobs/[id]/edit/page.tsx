  ...
  useEffect(() => {
    if (job && job._id === id && !loaded) {
      setForm({
        ...
        requiredSkills: job.requirements?.requiredSkills?.length ? job.requirements.requiredSkills : [""],
        preferredSkills: job.requirements?.preferredSkills?.length ? job.requirements.preferredSkills : [""],
        yearsOfExperience: job.requirements?.yearsOfExperience ?? 0,
        educationLevel: (job.requirements as any)?.educationLevel || "",
        niceToHave: job.niceToHave?.length ? job.niceToHave : [""],
        formQuestions: job.formQuestions?.length
          ? job.formQuestions.map((q) => q.question)
          : [""],
    ...
    return (
      <div className="max-w-2xl mx-auto px-6 py-12">
        <div className="animate-pulse space-y-4">
          ...
          <div className="h-8 w-64 bg-slate-200 dark:bg-white/[0.06] rounded" />
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12">
      <div>
        ...
        <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-10">Edit Job</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <Section label="Application Questions">
            <p className="text-[10px] text-slate-600 -mt-3">Questions applicants answer when applying.</p>
            <TagListField
              label=""
