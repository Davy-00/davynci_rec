...
export default function LandingPage() {
  return (
    <div className="min-h-screen dot-bg flex flex-col">
      <header className="h-14 flex items-center justify-between px-6 md:px-12 border-b border-white/[0.05]">
        ...
        <div className="hidden md:flex items-center gap-2 text-[10px] text-emerald-300 bg-emerald-400/10 border border-emerald-400/20 rounded-full px-3 py-1.5 font-semibold">
      ...
      <main className="flex-1 px-6 py-16 md:py-20">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-8 items-center">
          <section className="text-center lg:text-left">
            ...
            <h1 className="text-5xl md:text-7xl font-black text-white tracking-tight leading-none mb-5 fade-rise">
              AI Hiring Assistant
              <span className="block text-cyan-400">for Smart Recruiters</span>
            </h1>
            <p className="text-slate-400 text-base md:text-lg max-w-2xl leading-relaxed mb-8 fade-rise">
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-2.5 mb-8 fade-rise">
              {VALUE_PILLS.map(({ icon: Icon, label }) => (
                <span
                  key={label}
                  className="inline-flex items-center gap-1.5 text-[10px] text-slate-300 font-medium border border-white/[0.05] rounded-full px-3 py-1.5 bg-white/[0.02]"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-2xl fade-rise">
              <Link
                href="/portal"
                className="group relative flex flex-col items-start gap-4 bg-[#0d0d1a] border border-white/[0.07] hover:border-cyan-400/30 rounded-2xl p-6 text-left transition-all hover:bg-[#0d1520]"
              >
                ...
                <div>
                  <p className="text-white font-bold text-base mb-1">For Job Seekers</p>
              ...
              <Link
                href="/hr"
                className="group relative flex flex-col items-start gap-4 bg-[#0d0d1a] border border-white/[0.07] hover:border-slate-400/20 rounded-2xl p-6 text-left transition-all hover:bg-[#100d1a]"
              >
                ...
                <div>
                  <p className="text-white font-bold text-base mb-1">For Recruiters</p>
                  <p className="text-slate-500 text-xs leading-relaxed">
          ...
          <aside className="fade-rise bg-[#0d0d1a] border border-white/[0.06] rounded-3xl p-5 md:p-6">
            <p className="text-[10px] tracking-[0.28em] text-cyan-400/70 uppercase font-semibold mb-4">
              How it works
            </p>
            <div className="space-y-3">
              {FEATURE_CARDS.map((card) => (
                <div key={card.title} className="rounded-2xl border border-white/[0.05] bg-white/[0.02] p-4">
